...
# ---------------------------------------------------------------------------
# 常見申報提醒（自行整理，非健保署逐字公告）
# 這些是臨床/申報實務上常被問到的限制或慣例，官方支付標準的「通則」章節
# 才是最終依據；本檔只整理重點提醒使用者去核對，不能單獨作為申報依據。
...
TIPS = {
    ...
    "77026B": "品項名稱未標示單側／雙側，且無對應的雙側代碼。同次雙側輸尿管"
              "鏡碎石是否可各自全額申報、或次側需依「次要手術」折算計費，"
              "須依當年度支付標準總則之「同日同途徑執行多項手術」規定認定，"
              "不建議逕自以 2 倍點數申報，請先向院內健保／審查單位確認。",
    "77027B": "同 77026B：未標示單側／雙側，雙側同日施行時的折算方式請依"
              "支付標準總則及院內規定認定，不建議逕自乘以 2 申報。",
    "77028B": "同 77026B：未標示單側／雙側，雙側同日施行時的折算方式請依"
              "支付標準總則及院內規定認定，不建議逕自乘以 2 申報。另可加計"
              "一般材料費及雷射光纖 91%（見上方支付規範）。",
    "77001B": "官方名稱為「上或下三分之一輸尿管」，未標示單側／雙側；雙側"
              "同日施行的折算方式請依支付標準總則認定，不建議逕自乘以 2。",
    "77002B": "官方名稱為「中三分之一輸尿管」，未標示單側／雙側；雙側同日"
              "施行的折算方式請依支付標準總則認定，不建議逕自乘以 2。",
...
def main():
    ...
    with open(ROOT / "docs" / "urology-codes.csv", "w", encoding="utf-8-sig",
              newline="") as f:
        w = csv.writer(f)
        w.writerow(["大分類", "分類", "代碼", "中文名稱", "常用別名/縮寫",
                    "常見申報提醒(非官方)", "支付規範摘要"])
        for it in uro:
